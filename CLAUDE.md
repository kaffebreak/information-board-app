...
- `bousai_board.html`：15秒ごとに `/api/status` を取得して描画。取得が重なったときは通し番号で
  最新以外の応答を捨てる（古い応答で警報表示を平常に戻さない）。サイズ変更は300msまとめ。`fit()` がはみ出しを検知して文字縮小→詳細を1行化→詳細を隠す（`clip`）→「ほか n 件」。
...
- [ ] PC起動時の自動実行の手順確認（`start_board.bat` はサーバーが応答するまで待ってから開く。打ち切りはしない
  ――未起動のまま開くとキオスクがエラー画面で固まり、ページが読まれないので自動更新も動かないため。
  接続先ポートは `config.json` の `port` から読む。タスクスケジューラ／スタートアップへの登録手順は未確認）
